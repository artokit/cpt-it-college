...
public class PostRepository : IPostRepository
{
    ...
    public async Task<List<DbPost>> GetAllPublishedPosts()
    {
        var queryObject = new QueryObject(
            @"SELECT p.id, p.author_id as ""AuthorId"", idempotency_key as ""IdempotencyKey"", p.title as ""Title"", p.Content as ""Content"", p.created_at as ""CreatedAt"", p.updated_at as ""UpdatedAt"", p.Status,
    ...
    {
        var queryObject = new QueryObject(
            @"INSERT INTO posts(author_id, idempotency_key, title, content, created_at, updated_at, status) VALUES(@author_id, @idempotency_key, @title, @content, now(), now(), 'draft')
            ...
            new { author_id = dbPost.AuthorId, title=dbPost.Title, content=dbPost.Content, idempotency_key=dbPost.IdempotencyKey});
        var res = await dapperContext.CommandWithResponse<DbPost>(queryObject);
        return res;
    }

    public async Task<DbPost?> GetPostByIdempotencyKey(string idempotencyKey)
    {
        var queryObject = new QueryObject("SELECT * FROM POSTS WHERE idempotency_key=@idempotencyKey", new {idempotencyKey});
        return await dapperContext.FirstOrDefault<DbPost>(queryObject);
    }
